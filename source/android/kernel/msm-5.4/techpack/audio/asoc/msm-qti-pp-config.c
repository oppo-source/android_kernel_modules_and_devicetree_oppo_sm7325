...
static int auddet_ver_set(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	int ver_val = 0;

	ver_val = ucontrol->value.integer.value[0];
	pr_info("%s: set auddet version %d\n", __func__, ver_val);

	ret = adm_set_auddet_version_param(ver_val);
	ret = voice_set_auddet_version_param(ver_val);

	return ret;
}

static int auddet_ver_get(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	uint8_t ver_val = adm_get_auddet_version_param();

	ver_val = adm_get_auddet_version_param();
	pr_info("%s: enter: %d\n", __func__, ver_val);

	ucontrol->value.integer.value[0] = (int)ver_val;
...
static const struct snd_kcontrol_new oplus_auddet_mixer_controls[] = {
    ...
    SOC_SINGLE_EXT_TLV("AUDDET_VERSION", SND_SOC_NOPM, 0,
    INT_AUDDET_MAX_VAL, 0, auddet_ver_get,
    auddet_ver_set, auddet_max_value),
