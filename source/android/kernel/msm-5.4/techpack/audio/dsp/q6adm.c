...
int adm_set_auddet_version_param(int val)
{
	if(val != adm_auddet_version) {
		adm_auddet_version = val;
	}

	return 0;
}
EXPORT_SYMBOL(adm_set_auddet_version_param);

int adm_get_auddet_version_param(void)
{
	return adm_auddet_version;
}
EXPORT_SYMBOL(adm_get_auddet_version_param);
...
int adm_get_all_mute_pp_param_from_port(int port_id)
{
	...
	if(adm_auddet_version == 2) {
		param_hdr.module_id = MUTE_DETECT_MODULE_ID_RX;
		param_hdr.param_id = MUTE_DETECT_RESULT_PARAM_ID_RX;
	} else {
		param_hdr.module_id = MUTE_DETECT_MODULE_ID;
	...
	for (idx = 0; idx < MAX_COPPS_PER_PORT; idx++) {
		if (atomic_read(&this_adm.copp.id[port_idx][idx]) != RESET_COPP_ID) {
			...
			if (atomic_read(&this_adm.copp.session_type[port_idx][idx]) == SESSION_TYPE_TX) {
				if(adm_auddet_version == 2) {
					param_hdr.module_id = MUTE_DETECT_MODULE_ID_TX;
					param_hdr.param_id = MUTE_DETECT_RESULT_PARAM_ID_TX;
				}
				param_hdr.instance_id = 0x8000;
			}
