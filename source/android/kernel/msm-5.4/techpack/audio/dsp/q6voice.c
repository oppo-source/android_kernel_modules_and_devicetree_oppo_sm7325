...
#ifdef OPLUS_FEATURE_AUDIODETECT
int voice_set_auddet_version_param(int val)
{
	if(val != voice_auddet_version) {
		voice_auddet_version = val;
	}

	return 0;
}
EXPORT_SYMBOL(voice_set_auddet_version_param);
	...
	for (i = 0; i < MAX_VOC_SESSIONS; i++) {
		...
		if (is_voc_state_active(v->voc_state)) {
			for (j = 0; j < 2; j++) {
				...
				if (j == 0) {
					if(voice_auddet_version == 2) {
						param_hdr.module_id = MUTE_DETECT_MODULE_ID_RX;
						param_hdr.param_id = MUTE_DETECT_ENABLE_PARAM_ID_RX;
					}
					param_hdr.instance_id = INSTANCE_ID_0;
				} else {
					if(voice_auddet_version == 2) {
						param_hdr.module_id = MUTE_DETECT_MODULE_ID_TX;
						param_hdr.param_id = MUTE_DETECT_ENABLE_PARAM_ID_TX;
					}
...
int voice_get_cvp_param(void)
{
	...
	for (i = 0; i < MAX_VOC_SESSIONS; i++) {
		...
		for (j = 0; j < 2; j++) {
			...
			if (j == 0) {
				if(voice_auddet_version == 2) {
					get_param->module_id = MUTE_DETECT_MODULE_ID_RX;
					get_param->param_id = MUTE_DETECT_RESULT_PARAM_ID_RX;
				}
				get_param->instance_id = INSTANCE_ID_0;
			} else {
				if(voice_auddet_version == 2) {
					get_param->module_id = MUTE_DETECT_MODULE_ID_TX;
					get_param->param_id = MUTE_DETECT_RESULT_PARAM_ID_TX;
				}
