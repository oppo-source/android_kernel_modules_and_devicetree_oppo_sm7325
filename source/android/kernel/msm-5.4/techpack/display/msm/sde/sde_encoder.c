...
void sde_encoder_wakeup_ctl_prepare(struct drm_encoder *drm_enc)
{
	struct msm_drm_private *priv = NULL;
	struct sde_encoder_virt *sde_enc = NULL;
	struct msm_drm_thread *disp_thread;
	...
	if (!sde_enc->crtc ||
		sde_enc->crtc->index >= ARRAY_SIZE(priv->disp_thread)) {
		SDE_DEBUG_ENC(sde_enc,
			"invalid crtc:%d or crtc index:%d\n",
			sde_enc->crtc == NULL,
			sde_enc->crtc ? sde_enc->crtc->index : -EINVAL);
		return;
	}
	...
	if ( sde_enc->disp_info.display_type != SDE_CONNECTOR_SECONDARY)
		return;
	...
	if (sde_enc->rc_state == SDE_ENC_RC_STATE_IDLE) {
		...
		if (ret) {
			SDE_ERROR_ENC(sde_enc, "rc in state %d\n",
					sde_enc->rc_state);
			SDE_EVT32(DRMID(drm_enc), sde_enc->rc_state,
					SDE_EVTLOG_ERROR);
			goto end;
		}
...
int oplus_sync_panel_brightness(enum oplus_sync_method method, struct drm_encoder *drm_enc)
{
	...
	if ((!strcmp(display->display_type, "primary") &&
		...
		if ((ktime_to_us(ktime_sub(ktime_get(), last_te_timestamp)) % us_per_frame) > (us_per_frame - 800)) {
			usleep_range(800 + vsync_width, 1300 + vsync_width);
		}
