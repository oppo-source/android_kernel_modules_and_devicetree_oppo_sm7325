		...
		struct drm_crtc_state *old_state)
{
	...
	list_for_each_entry(encoder, &dev->mode_config.encoder_list, head) {
		...
		sde_encoder_wakeup_ctl_prepare(encoder);
...
extern struct hbm_status oplus_hbm_status;
		...
		struct plane_state *pstates, int cnt)
{
	...
	if (!strcmp(display->panel->oplus_priv.vendor_name, "S6E3HC3") ||
		...
		if (aod_index >= 0) {
			if (oplus_hbm_status.hbm_pvt_status || fp_mode
				|| oplus_request_power_status == 2) {
				for (i = 0; i < cnt; i++) {
					if (pstates[i].stage <= pstates[aod_index].stage)
						pstates[i].sde_pstate->is_skip = true;
				}
				aod_index = -1;
			}
		}
