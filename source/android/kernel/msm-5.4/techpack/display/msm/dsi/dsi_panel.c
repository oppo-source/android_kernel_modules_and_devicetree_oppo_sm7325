...
#ifdef OPLUS_BUG_STABILITY
/* PSW.MM.Display.Lcd.Stability, 2019-09-27, add log at display key evevnt */
	pr_info("<%s> debug for dsi_panel_reset\n", panel->oplus_priv.vendor_name);
#endif
...
#ifdef OPLUS_BUG_STABILITY
/* PSW.MM.Display.Lcd.Stability, 2019-09-27, add log at display key evevnt */
	pr_info("<%s> debug for dsi_panel_power_on\n", panel->oplus_priv.vendor_name);
#endif
...
#ifdef OPLUS_BUG_STABILITY
/* PSW.MM.Display.Lcd.Stability, 2019-09-27, add log at display key evevnt */
	pr_info("<%s> debug for dsi_panel_power_off\n", panel->oplus_priv.vendor_name);
#endif
	...
	if (OPLUS_DISPLAY_AOD_SCENE != get_oplus_display_scene()) {
		...
		if ((!strcmp(panel->name,"samsung S6E3HC3 dsc cmd mode panel"))) {
			...
			if (bl_lvl > PANEL_MAX_NOMAL_BRIGHTNESS) {
				...
				if(enable_global_hbm_flags == 1) {
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_END0_SWITCH);
					oplus_cmd_wait_vsync(drm_enc, panel);
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_END1_SWITCH);
					oplus_cmd_wait_vsync(drm_enc, panel);
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_END2_SWITCH);
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_END3_SWITCH);
					enable_global_hbm_flags = 0;
				}
			}
