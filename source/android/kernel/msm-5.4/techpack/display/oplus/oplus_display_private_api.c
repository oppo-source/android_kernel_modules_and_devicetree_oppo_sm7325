			       ...
			       size_t len)
{
	...
	if (is_set_seed && (display->panel->power_mode != SDE_MODE_DPMS_ON)) {
		pr_err("%s:panel off\n", __func__);
		goto done;
	}
		...
		const char *buf, size_t count)
{
	...
	if (sscanf(bufp, "%c%n", &read, &step) && read == 'r') {
		...
		ret = dsi_display_read_panel_reg(display, value, reg, len);
		if (ret < 0) {
			pr_err("%s dsi_display_read_panel_reg failed\n", __func__);
			return ret;
		}
			       ...
			       int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_ON:
		...
		if (display->panel->oplus_priv.is_osc_support) {
			if (!strcmp(display->panel->oplus_priv.vendor_name, "AMS662ZS01")
				|| (!strcmp(display->panel->oplus_priv.vendor_name, "S6E3XA1") && display->panel->panel_id2 > 6)) {
				dsi_update_dynamic_osc_clock();
			}
		}
