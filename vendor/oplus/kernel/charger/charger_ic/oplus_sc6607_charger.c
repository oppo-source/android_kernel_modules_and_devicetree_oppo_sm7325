...
static int sc6607_dpdm_irq_handle(struct sc6607 *chip)
{
	...
	pr_info("bc12_retry_num [%d] : prev_vbus_type[%d] --> cur_vbus_type[%d]\n",
			bc12_retry_num, prev_vbus_type, chip->vbus_type);
	if ((bc12_retry_num < OPLUS_BC12_RETRY_NUM) && (chip->vbus_type == SC6607_VBUS_TYPE_SDP ||
				chip->vbus_type == SC6607_VBUS_TYPE_CDP)) {
		bc12_retry_num++;
		if (bc12_retry_num != OPLUS_BC12_RETRY_NUM)
			chip->bc12.first_noti_sdp = true;
