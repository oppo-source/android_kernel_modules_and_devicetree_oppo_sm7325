...
extern void oplus_vooc_get_fastchg_started_pfunction(bool (*pfunc)(void));
#endif
#endif
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "qcom,ntc_pull_up_voltage", &val) >= 0)
		info->ntc_pull_up_voltage = val;
	else {
		chr_err("use default ntc_pull_up_voltage: %d\n",
			NTC_PULL_UP_VOLTAGE);
		info->ntc_pull_up_voltage =
					NTC_PULL_UP_VOLTAGE;
	}
...
int oplus_force_get_subboard_temp(void)
{
	...
	if (!is_param_init) {
		...
		ntc_param.i_tap_over_critical_low = 4397119;
		ntc_param.i_rap_pull_up_r = 100000;
		ntc_param.i_rap_pull_up_voltage = pinfo->ntc_pull_up_voltage;
		ntc_param.i_tap_min = -400;
		ntc_param.i_tap_max = 1250;
		ntc_param.i_25c_volt = 2457;
		ntc_param.pst_temp_table = sub_board_temp_table;
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	init_pps_key();
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0) && LINUX_VERSION_CODE < KERNEL_VERSION(5, 15, 0))
	oplus_vooc_get_fastchg_started_pfunction(&oplus_vooc_get_fastchg_started);
#endif
#endif
	chg_err(" done \n");
	return 0;
