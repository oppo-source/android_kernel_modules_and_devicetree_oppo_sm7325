...
static int sgm41512_inform_online_set(bool vbus_gd)
{
	int ret = 0;
...
enum charger_type mt_charger_type_detection_sgm41512(void)
{
	...
	for (i = 0; i < BC12_RETRY_TIMES; i++) {
		if ((MTK_CHR_Type_num == CHARGER_UNKNOWN || MTK_CHR_Type_num == STANDARD_HOST || MTK_CHR_Type_num == CHARGING_HOST) && pmic_chrdet_status()) {
