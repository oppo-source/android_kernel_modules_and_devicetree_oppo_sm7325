...
#define QCOM_EXCEPTION_HARDWARE_MODULE (8 << 8) //mtk have used 0-6; qcom use begin 8
...
#define KEVENT_FB_SNESOR_PROBE_FAILED(connext, msg, sensor)                               \
	do {                                                                              \
		snprintf(                                                                 \
			...
			acquire_event_field(EXCEP_PROBE),                                 \
			CAM_OLC_EXCEPTION_HEADER_ID |                                     \
				QCOM_EXCEPTION_HARDWARE_MODULE | EXCEP_PROBE,             \
			msg, sensor);                                                     \
...
#define KEVENT_FB_EEPRPOM_WR_FAILED(connext, msg, err)                                   \
	do {                                                                             \
		snprintf(                                                                \
			...
			acquire_event_field(EXCEP_I2C),                                  \
			CAM_OLC_EXCEPTION_HEADER_ID |                                    \
				QCOM_EXCEPTION_HARDWARE_MODULE | EXCEP_I2C,              \
			msg, err);                                                       \
